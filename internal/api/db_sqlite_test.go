...
func TestSQLiteDB_CreateLog(t *testing.T) {
	...
	if err := sqliteDB.CreateLog(ctx, log); err != nil {
		t.Fatalf("CreateLog failed: %v", err)
	}

	// Verify via count
	count, err := sqliteDB.GetLogCount(ctx, exec.ID)
	if err != nil {
		t.Fatalf("GetLogCount failed: %v", err)
	}

	if count != 1 {
		t.Errorf("Expected log count 1, got %d", count)
	}
}
...
func TestSQLiteDB_GetExecutionLogs(t *testing.T) {
	...
	levels := []LogLevel{LogLevelInfo, LogLevelDebug, LogLevelError}
	for i, level := range levels {
		log := LogEntry{
			ID:          "log_" + string(rune('1'+i)),
			ExecutionID: exec.ID,
			Level:       level,
			Message:     "Message " + string(rune('1'+i)),
		}
		if err := sqliteDB.CreateLog(ctx, log); err != nil {
			t.Fatalf("CreateLog %d failed: %v", i, err)
		}
	}

	// Get logs
	logs, total, err := sqliteDB.GetExecutionLogs(ctx, exec.ID, PaginationParams{Limit: 10, Offset: 0})
	if err != nil {
	...
	for i := 0; i < len(logs)-1; i++ {
		if logs[i].CreatedAt > logs[i+1].CreatedAt {
			t.Error("Expected logs to be ordered by created_at ASC")
		}
	}

	// Verify log levels
	if logs[0].Level != LogLevelInfo || logs[1].Level != LogLevelDebug || logs[2].Level != LogLevelError {
		t.Error("Expected logs to maintain correct levels")
	}
}
...
func TestSQLiteDB_DeleteExecution_CascadesLogs(t *testing.T) {
	...
	// Create function, version, execution, and logs
	fn := Function{
		ID:      "func_cascade_logs",
		Name:    "cascade-logs-test",
		EnvVars: make(map[string]string),
	}
	...
	for i := 0; i < 3; i++ {
		log := LogEntry{
			ID:          "log_cascade_" + string(rune('1'+i)),
			ExecutionID: exec.ID,
			Level:       LogLevelInfo,
			Message:     "Message",
		}
	...
	// Verify logs exist
	count, _ := sqliteDB.GetLogCount(ctx, exec.ID)
	if count != 3 {
		t.Fatalf("Expected 3 logs before deletion, got %d", count)
	}

	// Delete function (which cascades to execution)
	if err := sqliteDB.DeleteFunction(ctx, fn.ID); err != nil {
		t.Fatalf("DeleteFunction failed: %v", err)
	}
	...
	if len(logs) != 0 {
		t.Errorf("Expected logs to be cascade deleted, got %d", len(logs))
	}
	if total != 0 {
