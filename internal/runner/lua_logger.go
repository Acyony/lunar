...
func registerLogger(L *lua.LState, log logger.Logger, functionID string) {
	logTable := L.NewTable()

	// log.info(message)
	L.SetField(logTable, "info", L.NewFunction(func(L *lua.LState) int {
		message := L.CheckString(1)
		log.Info(functionID, message)
		return 0
	}))
